...
async function homeScreen() {
    const roles = (0, _header.getUserRoles)();
...
},{"../../components/main":"5zsxX","../../components/modais":"1Ukbc","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT","../session/components/tableSession":"2xVui","../session/components/componentsSession":"dEN24","../session/session":"cSjf2","../../components/header":"3QKkX"}],"5zsxX":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
function criarModalListaAlunosHTML(alunos = [], roles = [], presences = []) {
    const alunosRows = alunos.map((aluno)=>{
        ...
        if (roles.includes("ROLE_INSTRUCTOR")) return `
          <tr>
            <td>${nome}</td>
            <td>${cpf}</td>
            <td>${nascimento}</td>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content p-4">
          ...
          <div class="modal-footer d-flex justify-content-end gap-2">
            ${!(Array.isArray(roles) && roles.includes("ROLE_INSTRUCTOR")) ? `<button type="button" class="btn btn-outline-success" onclick="adicionarAluno()">Adicionar Aluno</button>` : `<button type="button" class="btn btn-outline-primary" onclick="salvarPresencas()">Salvar Presen\xe7as</button>`}
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "getUserRoles", ()=>getUserRoles);
...
async function fetchAulas() {
    ...
    if (roles.includes(!user.name)) try {
    ...
    try {
        ...
        if (roles.includes(user.name)) return data.filter((aula)=>aula.instructor === user.name && aula.isActive);
...
async function renderAgendamentoPage() {
    ...
    async function atualizarTabela() {
        ...
        const tableRows = aulas.map((aula)=>{
            ...
            } else if (roles.includes("ROLE_RECEPTIONIST")) acoes = `<button class="btn btn-sm btn-outline-success" onclick="abrirModalAlunos('${aulaId}')">Ver</button>`;
            else if (roles.includes("ROLE_INSTRUCTOR")) acoes = `<button class="btn btn-sm btn-secondary" onclick="abrirModalAlunos('${aulaId}')">Visualizar</button>`;
            return `
        <tr>
...
window.abrirModalAlunos = async function(id) {
    try {
        ...
        (0, _modais.criarModalListaAlunosHTML)(alunos, roles, presences);
...
window.mostrarModalConfirmacao = function(mensagem) {
    ...
    const modalLista = bootstrap.Modal.getInstance(document.getElementById("modalListaAlunos"));
...
window.removerAluno = async function(studentId) {
    ...
    try {
        ...
        if (response.ok) await atualizarModalAlunos(aulaSelecionadaId);
        else {
            const erro = await response.json();
...
window.salvarPresencas = async function() {
    try {
        ...
        checkboxes.forEach((checkbox)=>{
            if (checkbox.checked) presences.push(checkbox.dataset.alunoId);
        });
        if (!aulaSelecionadaId) {
            alert("Erro: Aula n\xe3o encontrada.");
            return;
        }
        ...
        if (response.ok) mostrarModalConfirmacao("Presen\xe7as registradas com sucesso!");
        else {
            const erro = await response.json();
            alert(erro.message || "Erro ao salvar presen\xe7as.");
        }
    } catch (error) {
        console.error("Erro ao salvar presen\xe7as:", error);
        alert("Erro ao processar presen\xe7a dos alunos.");
    }
};

},{"../../components/main":"5zsxX","../../components/modais":"1Ukbc","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT","../../components/header":"3QKkX","../../service/formatData":"iMnpH"}],"9vJvL":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
function loginScreen() {
    ...
    <section class="d-flex justify-content-center align-items-center vh-100">
      <div class="card p-4 w-50 w-sm-75 w-md-50 w-lg-25"">
        ...
        <form id="loginForm">
          <div class="mb-3">
            <label class="form-label">Usu\xe1rio</label>
